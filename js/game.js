...
window.updateGameSettings = function () {
  ...
  engine.gravity.y = settings.gravity / 10;
  bgMusic.volume = settings.musicVolume / 100;
  collisionSound.volume = settings.soundEffects / 100;
  explosionSound.volume = settings.soundEffects / 100;
...
function triggerVibration(patternName) {
  const settings = loadGameSettings();
  const vibrationEnabled = settings.vibration > 0;
  if (vibrationEnabled) {
...
function playSound(sound) {
  ...
  const soundVolume = settings.soundEffects / 100;
